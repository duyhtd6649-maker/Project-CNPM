...
class Notifications(AuditableModel):
    id = models.CharField(db_column='Id', primary_key=True, max_length=255,default=uuid.uuid4,editable=False)  # Field name made lowercase.
...
class Subscriptions(AuditableModel):
    id = models.CharField(db_column='Id', primary_key=True, max_length=255,default=uuid.uuid4,editable=False)  # Field name made lowercase.
...
class Interviews(AuditableModel):
    ...
    application = models.ForeignKey('Applications', on_delete=models.CASCADE, db_column='ApplicationId', blank=True, null=True, related_name='interviews')
